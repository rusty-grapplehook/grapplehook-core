...
const MEDIA_EXTS = [
  ".mp4", ".mkv", ".webm", ".mov", ".m4a", ".opus", ".ogg", ".aac", ".mp3",
];
...
export interface DownloadTask {
  ...
  emit<E extends keyof DownloadTaskEvents>(
    event: E,
    ...args: Parameters<DownloadTaskEvents[E]>
  ): boolean;
}
...
export class DownloadTask extends EventEmitter {
  ...
  private handleDownloadLine = (line: string): void => {
    ...
    if (p) this.emitProgress("download", p);
  };
    ...
    try {
      ...
      if (isTwoStage(this.opts)) {
        ...
        await transcodeToMp4(input, outputPath, this.opts, tools, {
          onProgress: (p) => {
            if (!this.cancelled) this.emitProgress("transcode", p);
          },
  ...
  private async resolveDownloader(tools: ResolvedTools): Promise<boolean> {
    if (this.opts.noAria2c) return false;
    if (this.opts.aria2c) return true;
